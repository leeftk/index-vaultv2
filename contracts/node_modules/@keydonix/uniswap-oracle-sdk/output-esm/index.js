import { rlpEncode, rlpDecode } from "./rlp-encoder.js";
import { stripLeadingZeros, addressToString, unsignedIntegerToUint8Array } from "./utils.js";
export async function getProof(eth_getStorageAt, eth_getProof, eth_getBlockByNumber, exchangeAddress, denominationToken, blockNumber) {
    ...
    if (denominationToken !== token0Address && denominationToken !== token1Address)
        throw new Error(`Denomination token ${addressToString(denominationToken)} is not one of the two tokens for the Uniswap exchange at ${addressToString(exchangeAddress)}`);
...
export async function getPrice(eth_getStorageAt, eth_getBlockByNumber, exchangeAddress, denominationToken, blockNumber) {
    async function getAccumulatorValue(innerBlockNumber, timestamp) {
        ...
        if (token0 !== denominationToken && token1 !== denominationToken)
            throw new Error(`Denomination token ${addressToString(denominationToken)} is not one of the tokens for exchange ${exchangeAddress}`);
        if (reserve0 === 0n)
